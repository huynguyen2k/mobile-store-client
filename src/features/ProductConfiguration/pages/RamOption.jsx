...
function RamOptionPage() {
	const [optionData, setOptionData] = useState(null)
	const [modalContent, setModalContent] = useState(null)
	const [isModalVisible, setIsModalVisible] = useState(false)

	const { loading, data, refetchData } = useFetchData(ramOptionApi.getAll)
	...
	const handleDeleteOption = async idList => {
		try {
			await sleep(1000)
			await Promise.all(idList.map(id => ramOptionApi.delete(id)))
			await Swal.fire({
				title: 'Thông báo!',
	...
	const handleAddOptionSubmit = async data => {
		try {
			await sleep(1000)
			const response = await ramOptionApi.add(data)
			await Swal.fire({
				title: 'Thông báo!',
	...
	const handleUpdateOptionSubmit = async data => {
		try {
			await sleep(1000)
			const response = await ramOptionApi.update(data)
			await Swal.fire({
				title: 'Thông báo!',
				text: response.message,
				icon: 'success',
